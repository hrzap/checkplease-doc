...
| status | server | Y | The check's (not the bundle's) status.  |
...
A check's status may be any of these values:

| Status | Complete? | Meaning |
|--|--|--|
| WHITE | N | The check is ordered or underway |
| ORANGE | N | The result is available and needs review  |
| GREEN | Y | The result is good |
| RED | Y | The result is bad |
| GREY | Y | The check was cancelled |
...
So for example, as long as your check is not yet complete, a change to the individual's first name would cause your web hook to be called, even if the check itself has not changed.
